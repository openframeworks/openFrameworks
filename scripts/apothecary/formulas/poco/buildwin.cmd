...
rem ACTION:        build|rebuild|clean|upgrade
...
echo ########################################################################
echo ####
echo #### STARTING VISUAL STUDIO BUILD (%VS_VERSION%, %PLATFORM%, %ACTION%)
echo ####
echo ########################################################################
echo.
echo.
	...
	echo upgrading %%G
	  if exist %%G (
		...
		for /f "tokens=1,2,3,4 delims=/" %%Q in ("%%G") do (
		  set PROJECT_FILE=%%Q%PLATFORM_SUFFIX%_vs120.%VCPROJ_EXT%
		  set TEST_PROJECT_FILE=testsuite/TestSuite%PLATFORM_SUFFIX%_vs120.%VCPROJ_EXT%
		 if exist !PROJECT_FILE! (
			copy %%Q%PLATFORM_SUFFIX%_vs120.* %%Q%PLATFORM_SUFFIX%_%VS_VERSION%.*
			set PROJECT_FILE=%%Q%PLATFORM_SUFFIX%_%VS_VERSION%.%VCPROJ_EXT%
			call :upgrade %%G 
			if ERRORLEVEL 1 goto buildfailed
		  )
		  set PROJECT_FILE=%%R%PLATFORM_SUFFIX%_vs120.%VCPROJ_EXT%
		  if exist !PROJECT_FILE! (
			copy %%R%PLATFORM_SUFFIX%_vs120.* %%R%PLATFORM_SUFFIX%_%VS_VERSION%.*
			set PROJECT_FILE=%%R%PLATFORM_SUFFIX%_%VS_VERSION%.%VCPROJ_EXT%
			call :upgrade %%G 
			if ERRORLEVEL 1 goto buildfailed
		  )
		  set PROJECT_FILE=%%S%PLATFORM_SUFFIX%_vs120.%VCPROJ_EXT%
		  if exist !PROJECT_FILE! (
			copy %%T%PLATFORM_SUFFIX%_vs120.* %%S%PLATFORM_SUFFIX%_%VS_VERSION%.*
			set PROJECT_FILE=%%S%PLATFORM_SUFFIX%_%VS_VERSION%.%VCPROJ_EXT%
			call :upgrade %%G 
			if ERRORLEVEL 1 goto buildfailed
		  )
		  set PROJECT_FILE=%%T%PLATFORM_SUFFIX%_vs120.%VCPROJ_EXT%
		  if exist !PROJECT_FILE! (
			copy %%T%PLATFORM_SUFFIX%_vs120.* %%T%PLATFORM_SUFFIX%_%VS_VERSION%.*
...
echo ------------------------------------------------------------------------
echo ------------------------------------------------------------------------
echo ---- Done upgrading [!PROJECT_FILE!]
echo ------------------------------------------------------------------------
echo ------------------------------------------------------------------------
echo.
