...
if [ -f /opt/vc/include/bcm_host.h ]; then
    ...
    apt-get ${FORCE_YES} install  gstreamer${GSTREAMER_VERSION}-omx
fi
...
if [ "$OS_CODENAME" = "7 (wheezy)" ]; then
    ...
    apt-get install ${FORCE_YES} g++-4.8
