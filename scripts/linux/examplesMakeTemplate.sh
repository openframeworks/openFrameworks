...
MAKEFILE_PATH=../templates/${PLATFORM}${ARCH}
echo "Makefile Path ${MAKEFILE_PATH}"

# echo "cd ${DIR}/../../examples"
# cd ${DIR}/../../examples
# cd ../../examples
...
if [ -d ${MAKEFILE_PATH} ];
echo ">> makefile folder exists"
then
	echo "listing folders in $EXAMPLES_FOLDER"
	for category in $(ls -1d `$EXAMPLES_FOLDER`)
	do
		echo "cat: " $category
		if [ $category != "addons" ] && [ $category != "ios" ] && [ $category != "android" ] && [[ -d $category ]]; then
			echo $category
			FOLDER=${EXAMPLES_FOLDER}/${category}
			echo examples/${category}
			# echo 
			# cd ${category}
			for example in $(ls -1d ${FOLDER})
			do
				EXAMPLE_FOLDER=${FOLDER}/${example}
				if [ -d ${EXAMPLE_FOLDER} ];
				then
					FULLPATH=${EXAMPLE_FOLDER}/${example}
					echo "	${FULLPATH}"
					# cp ${MAKEFILE_PATH}/{Makefile,config.make} ${example}
					# now overwriting
					# echo cp -rf ${MAKEFILE_PATH}/{Makefile,config.make} ${FULLPATH}
					# cp -rf ${MAKEFILE_PATH}/{Makefile,config.make} ${FULLPATH}
					# cp -rf `${MAKEFILE_PATH}/{Makefile,config.make}` `${FULLPATH}`
