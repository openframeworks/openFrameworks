...
class ofVec4f
{
  public:
    ...
    OF_DEPRECATED_MSG ("Use member method getInterpolated() instead.",
                       ofVec4f interpolated (const ofVec4f & pnt,
                                             float p) const);
...
inline
ofVec4f::ofVec4f (float _x, float _y, float _z, float _w):
x (_x),
y (_y),
z (_z),
w (_w)
...
inline void
ofVec4f::set (float _x, float _y, float _z, float _w)
{
    x = _x;
    y = _y;
    z = _z;
...
//
inline bool
     ofVec4f::operator== (const ofVec4f & vec) const const const const
     {
         return (x == vec.x) && (y == vec.y) && (z == vec.z) && (w == vec.w);
     }

     inline bool ofVec4f::operator!= (const ofVec4f & vec) const const const const
     {
         return (x != vec.x) || (y != vec.y) || (z != vec.z) || (w != vec.w);
     }

     inline bool ofVec4f::match (const ofVec4f & vec, float tolerance) const const const const
     {
...
//
//
     inline ofVec4f ofVec4f::operator+ (const ofVec4f & vec) const const const const
     {
...
inline ofVec4f
     ofVec4f::operator- (const float f) const const const const
     {
...
inline ofVec4f
     ofVec4f::operator- () const const const const
     {
...
inline ofVec4f
     ofVec4f::operator/ (const ofVec4f & vec) const const const const
     {
         return ofVec4f (vec.x != 0 ? x / vec.x : x,
                         vec.y != 0 ? y / vec.y : y,
                         vec.z != 0 ? z / vec.z : z,
                         vec.w != 0 ? w / vec.w : w);
     }
...
inline ostream &
operator<< (ostream & os, const ofVec4f & vec)
{
...
inline istream &
operator>> (istream & is, ofVec4f & vec)
{
    is >> vec.x;
    is.ignore (2);
    is >> vec.y;
...
inline ofVec4f
     ofVec4f::rescaled (const float length) const const const const
     {
         return getScaled (length);
     }

     inline ofVec4f ofVec4f::getScaled (const float length) const const const const
     {
...
//
inline float
     ofVec4f::distance (const ofVec4f & pnt) const const const const
     {
         float vx = x - pnt.x;
         float vy = y - pnt.y;
         float vz = z - pnt.z;
     ...
     inline float ofVec4f::distanceSquared (const ofVec4f & pnt) const const const const
     {
         return squareDistance (pnt);
     }

     inline float ofVec4f::squareDistance (const ofVec4f & pnt) const const const const
     {
         float vx = x - pnt.x;
         float vy = y - pnt.y;
         float vz = z - pnt.z;
 ...
 */
     inline ofVec4f ofVec4f::interpolated (const ofVec4f & pnt, float p) const const const const
     {
         return getInterpolated (pnt, p);
     }

     inline ofVec4f ofVec4f::getInterpolated (const ofVec4f & pnt, float p) const const const const
     {
         return ofVec4f (x * (1 - p) + pnt.x * p,
                         y * (1 - p) + pnt.y * p,
                         z * (1 - p) + pnt.z * p, w * (1 - p) + pnt.w * p);
     }
...
inline ofVec4f
     ofVec4f::middled (const ofVec4f & pnt) const const const const
     {
         return getMiddle (pnt);
     }

     inline ofVec4f ofVec4f::getMiddle (const ofVec4f & pnt) const const const const
     {
...
//
inline ofVec4f
     ofVec4f::normalized () const const const const
     {
         return getNormalized ();
     }

     inline ofVec4f ofVec4f::getNormalized () const const const const
     {
         float length = (float) sqrt (x * x + y * y + z * z + w * w);
         if (length > 0)
           {
               return ofVec4f (x / length, y / length, z / length,
                               w / length);
           }
         else
           {
...
//
inline ofVec4f
     ofVec4f::limited (float max) const const const const
     {
         return getLimited (max);
     }

     inline ofVec4f ofVec4f::getLimited (float max) const const const const
     {
         ofVec4f limited;
...
//
inline float
     ofVec4f::length () const const const const
     {
         return (float) sqrt (x * x + y * y + z * z + w * w);
     }

     inline float ofVec4f::lengthSquared () const const const const
     {
 ...
 * Dot Product.
 */
     inline float ofVec4f::dot (const ofVec4f & vec) const const const const
     {
         return x * vec.x + y * vec.y + z * vec.z + w * vec.w;
     }
...
inline ofVec4f
operator- (float f, const ofVec4f & vec)
{
    return ofVec4f (f - vec.x, f - vec.y, f - vec.z, f - vec.w);
}
