...
//      float _mat[4][4];
    ...
    void makeLookAtMatrix (const ofVec3f & eye, const ofVec3f & center,
                           const ofVec3f & up);
    ...
    void makeLookAtViewMatrix (const ofVec3f & eye, const ofVec3f & center,
                               const ofVec3f & up);
    ...
    inline static ofMatrix4x4 newTranslationMatrix (float x, float y,
                                                    float z);

    /// \sa makeRotationMatrix
    inline static ofMatrix4x4 newRotationMatrix (const ofVec3f & from,
                                                 const ofVec3f & to);
    inline static ofMatrix4x4 newRotationMatrix (float angle, float x,
                                                 float y, float z);
    inline static ofMatrix4x4 newRotationMatrix (float angle,
                                                 const ofVec3f & axis);
    inline static ofMatrix4x4 newRotationMatrix (float angle1,
                                                 const ofVec3f & axis1,
                                                 float angle2,
                                                 const ofVec3f & axis2,
                                                 float angle3,
    ...
    inline static ofMatrix4x4 newPerspectiveMatrix (double fovy,
                                                    double aspectRatio,
                                                    double zNear,
                                                    double zFar);
    ...
    void decompose (ofVec3f & translation,
                    ...
                    ofVec3f & scale, ofQuaternion & so) const;
    ...
    inline static ofVec3f transform3x3 (const ofVec3f & v,
                                        const ofMatrix4x4 & m);

    /// \brief Apply a 3x3 transform (no translation) of M * v.
    inline static ofVec3f transform3x3 (const ofMatrix4x4 & m,
                                        const ofVec3f & v);
...
inline bool
     ofMatrix4x4::isNaN () const const const const
     {
     ...
     inline ostream & operator<< (ostream & os, const ofMatrix4x4 & M)
{
    ...
    os << setw (w)
        << M._mat[0][0] << ", " << setw (w)
        << M._mat[0][1] << ", " << setw (w)
        << M._mat[0][2] << ", " << setw (w) << M._mat[0][3] << std::endl;

    os << setw (w)
        << M._mat[1][0] << ", " << setw (w)
        << M._mat[1][1] << ", " << setw (w)
        << M._mat[1][2] << ", " << setw (w) << M._mat[1][3] << std::endl;

    os << setw (w)
        << M._mat[2][0] << ", " << setw (w)
        << M._mat[2][1] << ", " << setw (w)
        << M._mat[2][2] << ", " << setw (w) << M._mat[2][3] << std::endl;

    os << setw (w)
        << M._mat[3][0] << ", " << setw (w)
        << M._mat[3][1] << ", " << setw (w)
        << M._mat[3][2] << ", " << setw (w) << M._mat[3][3];

    return os;
}

inline istream &
operator>> (istream & is, ofMatrix4x4 & M)
{
    is >> M._mat[0][0];
    is.ignore (2);
    is >> M._mat[0][1];
...
inline void
ofMatrix4x4::set (const ofMatrix4x4 & rhs)
{
    set (rhs.getPtr ());
}

inline void
ofMatrix4x4::set (float const *const ptr)
{
...
inline void
ofMatrix4x4::set (double const *const ptr)
{
    float *local_ptr = (float *) _mat;
    for (std::size_t i = 0; i < 16; ++i)
        local_ptr[i] = (float) ptr[i];
...
inline bool
     ofMatrix4x4::isIdentity () const const const const
     {
     ...
     inline void
         ofMatrix4x4::makeOrtho2DMatrix (double left, double right,
                                         double bottom, double top)
{
...
inline ofVec3f
     ofMatrix4x4::getTranslation () const const const const
     {
         return ofVec3f (_mat[3][0], _mat[3][1], _mat[3][2]);
     }

     inline ofVec3f ofMatrix4x4::getScale () const const const const
     {
...
inline ofMatrix4x4
ofMatrix4x4::newScaleMatrix (const ofVec3f & v)
{
    return newScaleMatrix (v.x, v.y, v.z);
}

inline ofMatrix4x4
ofMatrix4x4::newTranslationMatrix (float tx, float ty, float tz)
{
    ofMatrix4x4 m;
...
inline ofMatrix4x4
ofMatrix4x4::newTranslationMatrix (const ofVec3f & v)
{
    return newTranslationMatrix (v.x, v.y, v.z);
}

inline ofMatrix4x4
ofMatrix4x4::newRotationMatrix (const ofQuaternion & q)
{
...
inline ofMatrix4x4
ofMatrix4x4::newRotationMatrix (float angle1, const ofVec3f & axis1,
                                float angle2, const ofVec3f & axis2,
                                float angle3, const ofVec3f & axis3)
{
    ofMatrix4x4 m;
...
inline ofMatrix4x4
ofMatrix4x4::newOrthoMatrix (double left, double right,
                             double bottom, double top,
                             double zNear, double zFar)
{
    ofMatrix4x4 m;
...
inline ofMatrix4x4
ofMatrix4x4::newOrtho2DMatrix (double left, double right,
                               double bottom, double top)
{
    ofMatrix4x4 m;
...
inline ofMatrix4x4
ofMatrix4x4::newFrustumMatrix (double left, double right,
                               double bottom, double top,
                               double zNear, double zFar)
{
    ofMatrix4x4 m;
...
inline ofMatrix4x4
ofMatrix4x4::newPerspectiveMatrix (double fovy, double aspectRatio,
                                   double zNear, double zFar)
{
    ofMatrix4x4 m;
...
inline ofMatrix4x4
ofMatrix4x4::newLookAtMatrix (const ofVec3f & eye, const ofVec3f & center,
                              const ofVec3f & up)
{
    ofMatrix4x4 m;
...
inline ofVec3f
     ofMatrix4x4::postMult (const ofVec3f & v) const const const const
     {
         float d =
             1.0f / (_mat[3][0] * v.x + _mat[3][1] * v.y + _mat[3][2] * v.z +
                     _mat[3][3]);
           return
             ofVec3f ((_mat[0][0] * v.x + _mat[0][1] * v.y +
     ...
     inline ofVec3f ofMatrix4x4::preMult (const ofVec3f & v) const const const const
     {
         float d =
             1.0f / (_mat[0][3] * v.x + _mat[1][3] * v.y + _mat[2][3] * v.z +
                     _mat[3][3]);
           return
             ofVec3f ((_mat[0][0] * v.x + _mat[1][0] * v.y +
...
/// The vector is implicitly treated as a column-matrix
     inline ofVec4f ofMatrix4x4::postMult (const ofVec4f & v) const const const const
     {
         return
             ofVec4f ((_mat[0][0] * v.x + _mat[0][1] * v.y +
                       _mat[0][2] * v.z + _mat[0][3] * v.w),
                      (_mat[1][0] * v.x + _mat[1][1] * v.y +
                       _mat[1][2] * v.z + _mat[1][3] * v.w),
                      (_mat[2][0] * v.x + _mat[2][1] * v.y +
                       _mat[2][2] * v.z + _mat[2][3] * v.w),
                      (_mat[3][0] * v.x + _mat[3][1] * v.y +
                       _mat[3][2] * v.z + _mat[3][3] * v.w));
     }

/// \brief pre-multiplies the vector by the matrix (i.e. returns v mult M)
/// The vector is implicitly treated as a row-matrix
     inline ofVec4f ofMatrix4x4::preMult (const ofVec4f & v) const const const const
     {
         return
             ofVec4f ((_mat[0][0] * v.x + _mat[1][0] * v.y +
...
/// upper left 3x3 portion of the matrix (i.e. only the rotation part).
     inline ofVec3f
         ofMatrix4x4::transform3x3 (const ofVec3f & v, const ofMatrix4x4 & m)
{
    return
        ofVec3f ((m._mat[0][0] * v.x + m._mat[1][0] * v.y +
                  m._mat[2][0] * v.z),
                 (m._mat[0][1] * v.x + m._mat[1][1] * v.y +
                  m._mat[2][1] * v.z),
                 (m._mat[0][2] * v.x + m._mat[1][2] * v.y +
                  m._mat[2][2] * v.z));
}
...
/// upper left 3x3 portion of the matrix (i.e. only the rotation part).
inline ofVec3f
ofMatrix4x4::transform3x3 (const ofMatrix4x4 & m, const ofVec3f & v)
{
    return
        ofVec3f ((m._mat[0][0] * v.x + m._mat[0][1] * v.y +
                  m._mat[0][2] * v.z),
                 (m._mat[1][0] * v.x + m._mat[1][1] * v.y +
                  m._mat[1][2] * v.z),
                 (m._mat[2][0] * v.x + m._mat[2][1] * v.y +
                  m._mat[2][2] * v.z));
}
...
inline void
ofMatrix4x4::preMultTranslate (const ofVec3f & v)
{
...
inline void
ofMatrix4x4::postMultTranslate (const ofVec3f & v)
{
...
inline void
ofMatrix4x4::postMultTranslate (float x, float y, float z)
{
    if (x != 0)
      {
          _mat[0][0] += x * _mat[0][3];
...
inline void
ofMatrix4x4::preMultScale (const ofVec3f & v)
{
    _mat[0][0] *= v.getPtr ()[0];
    _mat[0][1] *= v.getPtr ()[0];
    _mat[0][2] *= v.getPtr ()[0];
...
inline void
ofMatrix4x4::postMultScale (const ofVec3f & v)
{
    _mat[0][0] *= v.getPtr ()[0];
    _mat[1][0] *= v.getPtr ()[0];
    _mat[2][0] *= v.getPtr ()[0];
...
inline void
ofMatrix4x4::rotate (const ofQuaternion & q)
{
    postMultRotate (q);
}
...
inline void
ofMatrix4x4::rotate (float angle, float x, float y, float z)
{
...
inline void
ofMatrix4x4::rotateRad (float angle, float x, float y, float z)
{
...
inline void
ofMatrix4x4::translate (float tx, float ty, float tz)
{
...
inline void
ofMatrix4x4::translate (const ofVec3f & v)
{
    postMultTranslate (v);
}
...
inline void
ofMatrix4x4::scale (float x, float y, float z)
{
    postMultScale (x, y, z);
}
...
inline void
ofMatrix4x4::scale (const ofVec3f & v)
{
    postMultScale (v);
}
...
inline void
ofMatrix4x4::glRotate (float angle, float x, float y, float z)
{
    preMultRotate (ofQuaternion (angle, ofVec3f (x, y, z)));
}

inline void
ofMatrix4x4::glRotateRad (float angle, float x, float y, float z)
{
    preMultRotate (ofQuaternion
                   (angle * static_cast < float >(RAD_TO_DEG),
                    ofVec3f (x, y, z)));
}

inline void
ofMatrix4x4::glRotate (const ofQuaternion & q)
{
    preMultRotate (q);
}

inline void
ofMatrix4x4::glTranslate (float tx, float ty, float tz)
{
    preMultTranslate (ofVec3f (tx, ty, tz));
}

inline void
ofMatrix4x4::glTranslate (const ofVec3f & v)
{
    preMultTranslate (v);
}

inline void
ofMatrix4x4::glScale (float x, float y, float z)
{
    preMultScale (ofVec3f (x, y, z));
}

inline void
ofMatrix4x4::glScale (const ofVec3f & v)
{
    preMultScale (v);
}
...
// AARON METHOD
inline void
ofMatrix4x4::postMultScale (float x, float y, float z)
{
    _mat[0][0] *= x;
    _mat[1][0] *= x;
    _mat[2][0] *= x;
...
inline void
ofMatrix4x4::preMultRotate (const ofQuaternion & q)
{
    if (q.zeroRotation ())
        return;
    ofMatrix4x4 r;
...
inline void
ofMatrix4x4::postMultRotate (const ofQuaternion & q)
{
    if (q.zeroRotation ())
        return;
    ofMatrix4x4 r;
...
// AARON METHOD
inline void
ofMatrix4x4::postMultRotate (float angle, float x, float y, float z)
{
    ofMatrix4x4 r;
...
inline ofVec3f
operator* (const ofVec3f & v, const ofMatrix4x4 & m)
{
    return m.preMult (v);
}
...
inline ofVec4f
operator* (const ofVec4f & v, const ofMatrix4x4 & m)
{
    return m.preMult (v);
}
