...
bool ofLineSegmentIntersection(const vectype & line1Start, const vectype & line1End, const vectype & line2Start, const vectype & line2End, vectype & intersection) {
	...
	if (
		(
			((diffLA.x * line2Start.y - diffLA.y * line2Start.x) <= compareA) ^ ((diffLA.x * line2End.y - diffLA.y * line2End.x) <= compareA))
		&& (((diffLB.x * line1Start.y - diffLB.y * line1Start.x) <= compareB) ^ ((diffLB.x * line1End.y - diffLB.y * line1End.x) <= compareB))) {
...
vectype ofCurvePoint(const vectype & a, const vectype & b, const vectype & c, const vectype & d, float t) {
	...
	float t2 = t * t;
	float t3 = t2 * t;
	pt.x = 0.5f * ((2.0f * b.x) + (-a.x + c.x) * t + (2.0f * a.x - 5.0f * b.x + 4 * c.x - d.x) * t2 + (-a.x + 3.0f * b.x - 3.0f * c.x + d.x) * t3);
	pt.y = 0.5f * ((2.0f * b.y) + (-a.y + c.y) * t + (2.0f * a.y - 5.0f * b.y + 4 * c.y - d.y) * t2 + (-a.y + 3.0f * b.y - 3.0f * c.y + d.y) * t3);
	return pt;
}
