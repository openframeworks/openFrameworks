...
class ofVec2f
{
  public:
    ...
    ofVec2f getMapped (const ofVec2f & origin,
                       const ofVec2f & vx, const ofVec2f & vy) const;
    ...
    OF_DEPRECATED_MSG ("Use member method getInterpolated() instead.",
                       ofVec2f interpolated (const ofVec2f & pnt,
                                             float p) const);

    // getMiddled
    OF_DEPRECATED_MSG ("Use member method getMiddled() instead.",
                       ofVec2f middled (const ofVec2f & pnt) const);

    // getMapped 
    OF_DEPRECATED_MSG ("Use member method getMapped() instead.",
                       ofVec2f mapped (const ofVec2f & origin,
                                       const ofVec2f & vx,
    ...
    // use getRotated
    OF_DEPRECATED_MSG ("Use member method getRotated() instead.",
                       ofVec2f rotated (float angle,
                                        const ofVec2f & pivot) const);
...
inline
ofVec2f::ofVec2f (float _x, float _y):
x (_x),
y (_y)
{
}
...
//
inline bool
     ofVec2f::operator== (const ofVec2f & vec) const const const const
     {
         return (x == vec.x) && (y == vec.y);
     }

     inline bool ofVec2f::operator!= (const ofVec2f & vec) const const const const
     {
         return (x != vec.x) || (y != vec.y);
     }

     inline bool ofVec2f::match (const ofVec2f & vec, float tolerance) const const const const
     {
...
// Tolerance is specified in degree.
     ...
     inline bool ofVec2f::isAligned (const ofVec2f & vec, float tolerance) const const const const
     {
         return fabs (this->angle (vec)) < tolerance;
     }

     inline bool ofVec2f::align (const ofVec2f & vec, float tolerance) const const const const
     {
         return isAligned (vec, tolerance);
     }

     inline bool ofVec2f::isAlignedRad (const ofVec2f & vec, float tolerance) const const const const
     {
         return fabs (this->angleRad (vec)) < tolerance;
     }

     inline bool ofVec2f::alignRad (const ofVec2f & vec, float tolerance) const const const const
     {
...
//
     ...
     inline ofVec2f ofVec2f::operator+ (const ofVec2f & vec) const const const const
     {
...
inline ofVec2f
     ofVec2f::operator/ (const ofVec2f & vec) const const const const
     {
         return ofVec2f (vec.x != 0 ? x / vec.x : x,
                         vec.y != 0 ? y / vec.y : y);
     }
...
inline ostream &
operator<< (ostream & os, const ofVec2f & vec)
{
    os << vec.x << ", " << vec.y;
    return os;
}
...
inline istream &
operator>> (istream & is, ofVec2f & vec)
{
    is >> vec.x;
    is.ignore (2);
    is >> vec.y;
...
inline ofVec2f
     ofVec2f::operator+ (const float f) const const const const
     {
...
inline ofVec2f
     ofVec2f::operator- () const const const const
     {
         return ofVec2f (-x, -y);
     }

     inline ofVec2f ofVec2f::operator* (const float f) const const const const
     {
         return ofVec2f (x * f, y * f);
     }
...
inline ofVec2f
     ofVec2f::rescaled (const float length) const const const const
     {
         return getScaled (length);
     }

     inline ofVec2f ofVec2f::getScaled (const float length) const const const const
     {
...
//
inline ofVec2f
     ofVec2f::rotated (float angle) const const const const
     {
         return getRotated (angle);
     }

     inline ofVec2f ofVec2f::getRotated (float angle) const const const const
     {
     ...
     inline ofVec2f ofVec2f::getRotatedRad (float angle) const const const const
     {
         float a = angle;
           return ofVec2f (x * cos (a) - y * sin (a),
                           x * sin (a) + y * cos (a));
...
inline ofVec2f
     ofVec2f::rotated (float angle, const ofVec2f & pivot) const const const const
     {
         return getRotated (angle, pivot);
     }

     inline ofVec2f ofVec2f::getRotated (float angle, const ofVec2f & pivot) const const const const
     {
         float a = (float) (angle * DEG_TO_RAD);
           return ofVec2f (((x - pivot.x) * cos (a) - (y - pivot.y) * sin (a))
                           + pivot.x,
                           ((x - pivot.x) * sin (a) +
                            (y - pivot.y) * cos (a)) + pivot.y);
     }
     ...
     inline ofVec2f & ofVec2f::rotate (float angle, const ofVec2f & pivot)
{
    float a = (float) (angle * DEG_TO_RAD);
    float xrot =
        ((x - pivot.x) * cos (a) - (y - pivot.y) * sin (a)) + pivot.x;
...
inline ofVec2f
     ofVec2f::getRotatedRad (float angle, const ofVec2f & pivot) const const const const
     {
         float a = angle;
           return ofVec2f (((x - pivot.x) * cos (a) - (y - pivot.y) * sin (a))
                           + pivot.x,
...
inline ofVec2f
     ofVec2f::mapped (const ofVec2f & origin,
                 const ofVec2f & vx,
                 const ofVec2f & vy) const const const const
     {
     ...
     inline ofVec2f &
         ofVec2f::map (const ofVec2f & origin, const ofVec2f & vx,
                       const ofVec2f & vy)
{
...
//
inline float
     ofVec2f::distance (const ofVec2f & pnt) const const const const
     {
         float vx = x - pnt.x;
         float vy = y - pnt.y;
...
//this method is deprecated in 006 please use squareDistance
     inline float ofVec2f::distanceSquared (const ofVec2f & pnt) const const const const
     {
         return squareDistance (pnt);
     }

     inline float ofVec2f::squareDistance (const ofVec2f & pnt) const const const const
     {
         float vx = x - pnt.x;
         float vy = y - pnt.y;
           return vx * vx + vy * vy;
...
// this method is deprecated in 006 please use getInterpolated
     inline ofVec2f ofVec2f::interpolated (const ofVec2f & pnt, float p) const const const const
     {
         return getInterpolated (pnt, p);
     }

     inline ofVec2f ofVec2f::getInterpolated (const ofVec2f & pnt, float p) const const const const
     {
...
inline ofVec2f
     ofVec2f::middled (const ofVec2f & pnt) const const const const
     {
         return getMiddle (pnt);
     }

     inline ofVec2f ofVec2f::getMiddle (const ofVec2f & pnt) const const const const
     {
...
//
inline ofVec2f
     ofVec2f::normalized () const const const const
     {
         return getNormalized ();
     }

     inline ofVec2f ofVec2f::getNormalized () const const const const
     {
...
//
inline ofVec2f
     ofVec2f::limited (float max) const const const const
     {
         return getLimited (max);
     }

     inline ofVec2f ofVec2f::getLimited (float max) const const const const
     {
         ofVec2f limited;
...
//
inline ofVec2f
     ofVec2f::perpendiculared () const const const const
     {
         return getPerpendicular ();
     }

     inline ofVec2f ofVec2f::getPerpendicular () const const const const
     {
         float length = (float) sqrt (x * x + y * y);
         if (length > 0)
...
//
inline float
     ofVec2f::length () const const const const
     {
         return (float) sqrt (x * x + y * y);
     }

     inline float ofVec2f::lengthSquared () const const const const
     {
         return (float) (x * x + y * y);
     }


     inline float ofVec2f::angle (const ofVec2f & vec) const const const const
     {
         return (float) (atan2 (x * vec.y - y * vec.x, x * vec.x + y * vec.y)
                         * RAD_TO_DEG);
     }

     inline float ofVec2f::angleRad (const ofVec2f & vec) const const const const
     {
         return atan2 (x * vec.y - y * vec.x, x * vec.x + y * vec.y);
     }


     inline float ofVec2f::dot (const ofVec2f & vec) const const const const
     {
         return x * vec.x + y * vec.y;
     }
...
inline ofVec2f
operator- (float f, const ofVec2f & vec)
{
    return ofVec2f (f - vec.x, f - vec.y);
}
