...
class ofVec3f
{
  public:
    ...
    ofVec3f getRotated (float angle, const ofVec3f & pivot,
    ...
    ofVec3f getRotatedRad (float angle, const ofVec3f & pivot,
                           const ofVec3f & axis) const;
    ...
    ofVec3f & rotate (float angle, const ofVec3f & pivot,
                      const ofVec3f & axis);
    ...
    /// using 'pivot' as the origin of rotation.
    ofVec3f & rotateRad (float angle, const ofVec3f & pivot,
                         const ofVec3f & axis);
    ...
    ofVec3f & map (const ofVec3f & origin,
                   const ofVec3f & vx,
                   const ofVec3f & vy, const ofVec3f & vz);
    ...
    // getScaled
    OF_DEPRECATED_MSG ("Use member method getScaled() instead.",
                       ofVec3f rescaled (const float length) const);

    // scale
    OF_DEPRECATED_MSG ("Use member method scale() instead.",
    ...
    // getNormalized
    OF_DEPRECATED_MSG ("Use member method getNormalized() instead.",
                       ofVec3f normalized ()const);

    // getLimited
    OF_DEPRECATED_MSG ("Use member method getLimited() instead.",
                       ofVec3f limited (float max) const);

    // getCrossed
    OF_DEPRECATED_MSG ("Use member method getCrossed() instead.",
    ...
    OF_DEPRECATED_MSG ("Use member method getMapped() instead.",
                       ofVec3f mapped (const ofVec3f & origin,
                                       const ofVec3f & vx, const ofVec3f & vy,
                                       const ofVec3f & vz) const);

    // use squareDistance
    OF_DEPRECATED_MSG ("Use member method squareDistance() instead.",
                       float distanceSquared (const ofVec3f & pnt) const);

    // use getInterpolated
    OF_DEPRECATED_MSG ("Use member method getInterpolated() instead.",
                       ofVec3f interpolated (const ofVec3f & pnt,
                                             float p) const);

    // use getMiddle
    OF_DEPRECATED_MSG ("Use member method getMiddle() instead.",
                       ofVec3f middled (const ofVec3f & pnt) const);

    // use getRotated
    OF_DEPRECATED_MSG ("Use member method getRotated() instead.",
                       ofVec3f rotated (float angle, const ofVec3f & pivot,
                                        const ofVec3f & axis) const);
...
inline
ofVec3f::ofVec3f (float _x, float _y, float _z):
x (_x),
y (_y),
z (_z)
{
...
inline void
ofVec3f::set (float _x, float _y, float _z)
{
    x = _x;
    y = _y;
    z = _z;
...
//
inline bool
     ofVec3f::operator== (const ofVec3f & vec) const const const const
     {
         return (x == vec.x) && (y == vec.y) && (z == vec.z);
     }

     inline bool ofVec3f::operator!= (const ofVec3f & vec) const const const const
     {
         return (x != vec.x) || (y != vec.y) || (z != vec.z);
     }

     inline bool ofVec3f::match (const ofVec3f & vec, float tolerance) const const const const
     {
 ...
 * Checks if vectors look in the same direction.
 */
     inline bool ofVec3f::isAligned (const ofVec3f & vec, float tolerance) const const const const
     {
         float angle = this->angle (vec);
           return angle < tolerance;
     }

     inline bool ofVec3f::align (const ofVec3f & vec, float tolerance) const const const const
     {
         return isAligned (vec, tolerance);
     }

     inline bool ofVec3f::isAlignedRad (const ofVec3f & vec, float tolerance) const const const const
     {
         float angle = this->angleRad (vec);
           return angle < tolerance;
     }

     inline bool ofVec3f::alignRad (const ofVec3f & vec, float tolerance) const const const const
     {
...
inline istream &
operator>> (istream & is, ofVec3f & vec)
{
    is >> vec.x;
    is.ignore (2);
    is >> vec.y;
...
inline ofVec3f
     ofVec3f::operator+ (const ofVec3f & pnt) const const const const
     {
...
inline ofVec3f
     ofVec3f::operator/ (const ofVec3f & vec) const const const const
     {
         return ofVec3f (vec.x != 0 ? x / vec.x : x,
                         vec.y != 0 ? y / vec.y : y,
                         vec.z != 0 ? z / vec.z : z);
     }
...
inline ofVec3f
     ofVec3f::operator- () const const const const
     {
...
//}
     ...
     inline ofVec3f ofVec3f::operator+ (const float f) const const const const
     {
...
inline ofVec3f
     ofVec3f::operator- (const float f) const const const const
     {
         return ofVec3f (x - f, y - f, z - f);
     }
...
//
inline ofVec3f
     ofVec3f::rescaled (const float length) const const const const
     {
         return getScaled (length);
     }

     inline ofVec3f ofVec3f::getScaled (const float length) const const const const
     {
         float l = (float) sqrt (x * x + y * y + z * z);
         if (l > 0)
               return ofVec3f ((x / l) * length, (y / l) * length,
                               (z / l) * length);
         else
               return ofVec3f ();
     }
...
//
inline ofVec3f
     ofVec3f::rotated (float angle, const ofVec3f & axis) const const const const
     {
         return getRotated (angle, axis);
     }

     inline ofVec3f ofVec3f::getRotated (float angle, const ofVec3f & axis) const const const const
     {
     ...
     inline ofVec3f ofVec3f::getRotatedRad (float angle, const ofVec3f & axis) const const const const
     {
         ofVec3f ax = axis.getNormalized ();
         float a = angle;
         float sina = sin (a);
...
inline ofVec3f
     ofVec3f::getRotated (float ax, float ay, float az) const const const const
     {
         ...
         float ny =
             (a * f + b * d * e) * x + (a * e - b * d * f) * y - b * c * z;
         float nz =
             (b * f - a * d * e) * x + (a * d * f + b * e) * y + a * c * z;

           return ofVec3f (nx, ny, nz);
     }

     inline ofVec3f ofVec3f::getRotatedRad (float ax, float ay, float az) const const const const
     {
         float a = cos (ax);
         float b = sin (ax);
         float c = cos (ay);
...
//
inline ofVec3f
     ofVec3f::rotated (float angle, const ofVec3f & pivot, const ofVec3f & axis) const const const const
     {
     ...
     inline ofVec3f
         ofVec3f::getRotated (float angle, const ofVec3f & pivot,
                              const ofVec3f & axis) const const const const
     {
         ofVec3f ax = axis.getNormalized ();
         float tx = x - pivot.x;
         float ty = y - pivot.y;
     ...
     inline ofVec3f
         ofVec3f::getRotatedRad (float angle,
                                 const ofVec3f & pivot,
                                 const ofVec3f & axis) const const const const
     {
         ofVec3f ax = axis.getNormalized ();
...
inline ofVec3f
     ofVec3f::mapped (const ofVec3f & origin,
                 const ofVec3f & vx,
                 const ofVec3f & vy,
                 const ofVec3f & vz) const const const const
     {
...
//
inline float
     ofVec3f::distance (const ofVec3f & pnt) const const const const
     {
         float vx = x - pnt.x;
         float vy = y - pnt.y;
         float vz = z - pnt.z;
     ...
     inline float ofVec3f::distanceSquared (const ofVec3f & pnt) const const const const
     {
         return squareDistance (pnt);
     }

     inline float ofVec3f::squareDistance (const ofVec3f & pnt) const const const const
     {
         float vx = x - pnt.x;
         float vy = y - pnt.y;
         float vz = z - pnt.z;
 ...
 */
     ...
     inline ofVec3f ofVec3f::interpolated (const ofVec3f & pnt, float p) const const const const
     {
         return getInterpolated (pnt, p);
     }

     inline ofVec3f ofVec3f::getInterpolated (const ofVec3f & pnt, float p) const const const const
     {
         return ofVec3f (x * (1 - p) + pnt.x * p,
                         y * (1 - p) + pnt.y * p, z * (1 - p) + pnt.z * p);
     }
...
inline ofVec3f
     ofVec3f::middled (const ofVec3f & pnt) const const const const
     {
         return getMiddle (pnt);
     }

     inline ofVec3f ofVec3f::getMiddle (const ofVec3f & pnt) const const const const
     {
         return ofVec3f ((x + pnt.x) / 2.0f, (y + pnt.y) / 2.0f,
                         (z + pnt.z) / 2.0f);
     }
...
//
inline ofVec3f
     ofVec3f::normalized () const const const const
     {
         return getNormalized ();
     }

     inline ofVec3f ofVec3f::getNormalized () const const const const
     {
...
inline ofVec3f
     ofVec3f::limited (float max) const const const const
     {
         return getLimited (max);
     }

     inline ofVec3f ofVec3f::getLimited (float max) const const const const
     {
         ofVec3f limited;
...
//
inline ofVec3f
     ofVec3f::crossed (const ofVec3f & vec) const const const const
     {
         return getCrossed (vec);
     }

     inline ofVec3f ofVec3f::getCrossed (const ofVec3f & vec) const const const const
     {
 ...
 */
inline ofVec3f
     ofVec3f::perpendiculared (const ofVec3f & vec) const const const const
     {
         return getPerpendicular (vec);
     }

     inline ofVec3f ofVec3f::getPerpendicular (const ofVec3f & vec) const const const const
     {
         ...
         if (length > 0)
               return ofVec3f (crossX / length, crossY / length,
                               crossZ / length);
         else
               return ofVec3f ();
     }
...
//
inline float
     ofVec3f::length () const const const const
     {
         return (float) sqrt (x * x + y * y + z * z);
     }

     inline float ofVec3f::lengthSquared () const const const const
     {
 ...
 */
     inline float ofVec3f::angle (const ofVec3f & vec) const const const const
     {
     ...
     inline float ofVec3f::angleRad (const ofVec3f & vec) const const const const
     {
         ofVec3f n1 = this->getNormalized ();
         ofVec3f n2 = vec.getNormalized ();
 ...
 * Dot Product.
 */
     inline float ofVec3f::dot (const ofVec3f & vec) const const const const
     {
         return x * vec.x + y * vec.y + z * vec.z;
     }
...
inline ofVec3f
operator- (float f, const ofVec3f & vec)
{
    return ofVec3f (f - vec.x, f - vec.y, f - vec.z);
}
