...
///
class ofBitmapString{

    public:

		ofBitmapString(const ofPoint & p){
			pos = p;
		}
		ofBitmapString(float x, float y, float z=0.0f){
			pos.set(x, y, z);
		}
        ...
        template <class T> 
		ofBitmapString& operator<<(const T& value){
            message << value;
            return *this;
        }

        /// catch the << ostream function pointers such as std::endl and std::hex
        ofBitmapString& operator<<(std::ostream& (*func)(std::ostream&)){
			func(message);
            return *this;
        }
	...
	private:
		...
		ofBitmapString(ofBitmapString const&) {}        		// not defined, not copyable
        ofBitmapString& operator=(ofBitmapString& from) {return *this;}	// not defined, not assignable
};
