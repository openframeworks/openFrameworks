...
public:
	ofBitmapFont();
	~ofBitmapFont();
	ofMesh getMesh(const std::string & text, int x, int y, ofBitmapMode mode=OF_BITMAPMODE_MODEL_BILLBOARD, bool vFlipped=true) const;
	const ofTexture & getTexture() const;
	ofRectangle getBoundingBox(const std::string & text, int x, int y, ofBitmapMode mode = ofGetStyle().bitmapMode, bool vFlipped = ofIsVFlipped()) const;
private:
	static void init();
