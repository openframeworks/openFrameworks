...
class ofCairoRenderer:public ofBaseRenderer
{
  public:
    ...
    void setup (string filename, Type type =
                ofCairoRenderer::FROM_FILE_EXTENSION, bool multiPage =
                true, bool b3D = false, ofRectangle outputsize =
                ofRectangle (0, 0, 0, 0));
    void setupMemoryOnly (Type _type, bool multiPage = true, bool b3D =
                          false, ofRectangle viewport =
                          ofRectangle (0, 0, 0, 0));
    void close ();
    void flush ();
    ...
    void draw (const ofMesh & vertexData, ofPolyRenderMode mode,
    ...
    void draw (const vector < ofPoint > &vertexData,
               ofPrimitiveMode drawMode) const;
    void draw (const ofImage & img, float x, float y, float z, float w,
  ...
  private:
    ...
    static _cairo_status stream_function (void *closure,
                                          const unsigned char *data,
                                          unsigned int length);
    void draw (const ofPixels & img, float x, float y, float z, float w,
               float h, float sx, float sy, float sw, float sh) const;
