...
public:	
	...
	static void tessellateToMesh( const vector<ofPolyline>& src, ofPolyWindingMode polyWindingMode, vector<ofMesh> & dstmesh, bool bIs2D=false );
	static void tessellateToMesh( const ofPolyline& src,  ofPolyWindingMode polyWindingMode, vector<ofMesh>& dstmesh, bool bIs2D=false );
...
private:
	...
	friend void * cacheAllocator( void *userData, unsigned int size );
	friend void * cacheReallocator( void *userData, void* ptr, unsigned int size );
};
