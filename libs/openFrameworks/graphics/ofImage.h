...
void ofSaveImage (ofPixels & pix, ofBuffer & buffer, ofImageFormat format =
                  OF_IMAGE_FORMAT_PNG, ofImageQualityType qualityLevel =
                  OF_IMAGE_QUALITY_BEST);

/// \todo Needs documentation.
void ofSaveImage (ofFloatPixels & pix, string path,
                  ofImageQualityType qualityLevel = OF_IMAGE_QUALITY_BEST);
void ofSaveImage (ofFloatPixels & pix, ofBuffer & buffer,
                  ofImageFormat format =
                  OF_IMAGE_FORMAT_PNG, ofImageQualityType qualityLevel =
                  OF_IMAGE_QUALITY_BEST);

/// \todo Needs documentation.
void ofSaveImage (ofShortPixels & pix, string path,
                  ofImageQualityType qualityLevel = OF_IMAGE_QUALITY_BEST);
void ofSaveImage (ofShortPixels & pix, ofBuffer & buffer,
                  ofImageFormat format =
                  OF_IMAGE_FORMAT_PNG, ofImageQualityType qualityLevel =
                  OF_IMAGE_QUALITY_BEST);
...
template < typename PixelType > class ofImage_:public ofBaseImage_ <
    PixelType >
{
  public:
    ...
    OF_DEPRECATED_MSG ("Use load instead",
    ...
    void drawSubsection (float x, float y, float w, float h, float sx,
                         float sy) const;
    ...
    /// \param sx X position in image to begin cropping from.
    /// \param sy Y position in image to begin cropping from.
    void drawSubsection (float x, float y, float z, float w, float h,
    ...
    void setFromPixels (const PixelType * pixels, int w, int h,
                        ofImageType type, bool bOrderIsRGB = true);
    ...
    /// \param w Width of region to crop.
    /// \param h Height of region to crop.
    void cropFrom (ofImage_ < PixelType > &otherImage, int x, int y, int w,
                   int h);
    ...
    void save (string fileName, ofImageQualityType compressionLevel =
               OF_IMAGE_QUALITY_BEST);
    ...
    /// \param compressionLevel The ofImageQualityType.
    void save (ofBuffer & buffer, ofImageQualityType compressionLevel =
               OF_IMAGE_QUALITY_BEST);
    ...
    void save (const ofFile & file, ofImageQualityType compressionLevel =
               OF_IMAGE_QUALITY_BEST);

    OF_DEPRECATED_MSG ("Use save instead",
                       void saveImage (string fileName,
                                       ofImageQualityType compressionLevel =
                                       OF_IMAGE_QUALITY_BEST));
    OF_DEPRECATED_MSG ("Use save instead",
  ...
  protected:
    ...
    void resizePixels (ofPixels_ < PixelType > &pix, int newWidth,
                       int newHeight);
    void unloadTexture ();
...
template < typename PixelType >
    ...
    PixelType >::operator= (const ofImage_ < SrcType > &mom)
{
    clone (mom);
    update ();
    return *this;
    ...
    void ofImage_ < PixelType >::clone (const ofImage_ < SrcType > &mom)
{
    ...
    if (bUseTexture == true && mom.getTexture ().isAllocated ())
      {
          tex.allocate (pixels.getWidth (), pixels.getHeight (),
                        ofGetGlInternalFormat (pixels));
      }
