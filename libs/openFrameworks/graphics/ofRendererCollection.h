...
class ofRendererCollection:public ofBaseRenderer
{
  public:
        ...
        for (int i = 0; i < (int) renderers.size (); i++)
          {
              if (renderers[i]->getType () == "GL"
                  || renderers[i]->getType () == "ProgrammableGL")
                {
               ...
               bool useColors, bool useTextures, bool useNormals) const
    {
        for (int i = 0; i < (int) renderers.size (); i++)
          {
              renderers[i]->draw (vertexData, mode, useColors, useTextures,
                                  useNormals);
          }
    }
    ...
    void draw (const ofFloatImage & img, float x, float y, float z, float w,
               float h, float sx, float sy, float sw, float sh) const
    {
        for (int i = 0; i < (int) renderers.size (); i++)
          {
    ...
    void draw (const ofShortImage & img, float x, float y, float z, float w,
               float h, float sx, float sy, float sw, float sh) const
    {
        for (int i = 0; i < (int) renderers.size (); i++)
          {
                                 ...
                                 0, float farDist = 0)
    {
        for (int i = 0; i < (int) renderers.size (); i++)
          {
              renderers[i]->setupScreenPerspective (width, height, fov,
                                                    nearDist, farDist);
          }
    }
                           ...
                           -1, float nearDist = -1, float farDist = 1)
    {
        for (int i = 0; i < (int) renderers.size (); i++)
          {
              renderers[i]->setupScreenOrtho (width, height, nearDist,
                                              farDist);
          }
    }
    ...
    bool isVFlipped () const
    {
        if (!renderers.empty ())
          ...
          {
              ofLogWarning () <<
                  "No renderer in renderer collection, but vflipped requested returning true.";
              return true;
          }
    }
