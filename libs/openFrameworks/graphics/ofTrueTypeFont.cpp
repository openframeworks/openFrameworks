				...
				iterateString(str, 0, 0, vflip, [&](uint32_t c, ofVec2f) {
					try {
						if (c != '\n') {
							auto g = loadGlyph(c);
							if (c == '\t')
								lineWidth += g.props.advance + getGlyphProperties(' ').advance * spaceSize * TAB_WIDTH;
							else if (c == ' ')
								lineWidth += g.props.advance + getGlyphProperties(' ').advance * spaceSize;
							else if (isValidGlyph(c))
								lineWidth += g.props.advance + getGlyphProperties(' ').advance * (letterSpacing - 1.f);
