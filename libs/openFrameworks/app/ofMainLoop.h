...
class ofMainLoop
{
  public:
    ...
    shared_ptr < ofAppBaseWindow >
        createWindow (const ofWindowSettings & settings);
      template < typename Window > void addWindow (shared_ptr < Window >
                                                   window)
    {
        ...
        ofAddListener (window->events ().keyPressed, this,
                       &ofMainLoop::keyPressed);
    }

    void run (shared_ptr < ofAppBaseWindow > window,
  ...
  private:
    ...
    map < shared_ptr < ofAppBaseWindow >,
        shared_ptr < ofBaseApp > >windowsApps;
    bool bShouldClose;
