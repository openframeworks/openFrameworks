...
if ( CMAKE_SYSTEM_NAME MATCHES "Linux" ) 
    ...
    if ( OPENFRAMEWORKS_USE_UNICAP_VIDCAP )
      ...
      if ( UNICAP_LIBRARIES )
          set ( OPENFRAMEWORKS_LIBRARIES ${OPENFRAMEWORKS_LIBRARIES} ${UNICAP_LIBRARIES} ${UNICAP_LDFLAGS} ${UNICAP_LDFLAGS_OTHER} )
          set ( OPENFRAMEWORKS_INCLUDES ${OPENFRAMEWORKS_INCLUDES} ${UNICAP_INCLUDE_DIRS} ) 
          set (OPENFRAMEWORKS_CXX_FLAGS " ${OPENFRAMEWORKS_CXX_FLAGS} ${UNICAP_CFLAGS}")
      else()
    ...
    if (DEFINED UDEV_FOUND)
        set (OPENFRAMEWORKS_LIBRARIES ${OPENFRAMEWORKS_LIBRARIES} ${UDEV_LIBRARIES} ${UDEV_LDFLAGS} ${UDEV_LDFLAGS_OTHER})
        set (OPENFRAMEWORKS_INCLUDES ${OPENFRAMEWORKS_INCLUDES} ${UDEV_INCLUDE_DIRS})
        set (OPENFRAMEWORKS_CXX_FLAGS ${OPENFRAMEWORKS_CXX_FLAGS} ${UDEV_CFLAGS})
    else()
        message ( FATAL_ERROR "Cannot find udev")
    endif()

    pkg_search_module(GST REQUIRED gstreamer-app-0.10 gstreamer-video-0.10)
    if ( GST_LIBRARIES ) 
        set (  OPENFRAMEWORKS_LIBRARIES ${OPENFRAMEWORKS_LIBRARIES}  ${GST_LIBRARIES} ${GST_LDFLAGS} ${GST_LDFLAGS_OTHER} )
        set ( OPENFRAMEWORKS_INCLUDES ${OPENFRAMEWORKS_INCLUDES} ${GST_INCLUDE_DIRS} ) 
        set (OPENFRAMEWORKS_CXX_FLAGS ${OPENFRAMEWORKS_CXX_FLAGS} ${GST_CFLAGS})
    else()
        message( FATAL_ERROR "Cannot find Gstreamer-app")
    endif()
    
    pkg_search_module(GSTVIDEO REQUIRED gstreamer-video-0.10)
    if ( GSTVIDEO_LIBRARIES ) 
        ...
        set (OPENFRAMEWORKS_CXX_FLAGS ${OPENFRAMEWORKS_CXX_FLAGS} ${GSTVIDEO_CFLAGS})
    else()
    ...
    if ( AVFORMAT_LIBRARIES)
        ...
        set (OPENFRAMEWORKS_CXX_FLAGS ${OPENFRAMEWORKS_CXX_FLAGS} ${AVFORMAT_CFLAGS})
    else()
    ...
    if ( SWSCALE_LIBRARIES)
        ...
        set (OPENFRAMEWORKS_CXX_FLAGS ${OPENFRAMEWORKS_CXX_FLAGS} ${SWSCALE_CFLAGS})
    else()
...
foreach(inc ${OPENFRAMEWORKS_CXX_FLAGS})
  set(CMAKE_CXX_FLAGS " ${inc}")
endforeach()
