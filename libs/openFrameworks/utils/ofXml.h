...
class ofXml:public ofBaseFileSerializer
{
  ...
  public:
    ...
    template < class T > bool addValue (const string & path, T data =
                                        T (), bool createEntirePath = false)
    {
        ...
        if (tokens.size () > 1)
          {
              ...
              Poco::XML::Element * firstElement = nullptr, *lastElement =
                  nullptr;
              if (element)
                {
                    lastElement = element;
              ...
              for (std::size_t i = 0; i < tokens.size (); i++)
                {
                    Poco::XML::Element * newElement =
                        getPocoDocument ()->createElement (tokens.at (i));
              ...
              if (value != "")
                {
                    ...
                    Poco::XML::Text * text =
                        getPocoDocument ()->createTextNode (value);
                    try
                    {
                    ...
                    {
                        ofLogError ("ofxXml") <<
                            "addValue(): couldn't set node value: " <<
                            DOMErrorMessage (e.code ());
                        return false;
                    }
                }
        ...
        else
          {
              ...
              Poco::XML::Element * newElement =
                  getPocoDocument ()->createElement (path);
              ...
              if (path == "")
                {
                    if (element->firstChild ()
                        ...
                        Poco::XML::Node::TEXT_NODE)
                      {
                          return ofFromString < T > (element->innerText ());
                      }
                    else
                      {
                          ofLogWarning ("ofXml") << "getValue(): path \"" <<
                              path << "\" not found when getting value";
                ...
                {
                    Poco::XML::Element * e =
                        (Poco::XML::Element *) element->getNodeByPath (path);
                    if (e)
                      {
