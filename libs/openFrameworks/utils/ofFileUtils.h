...
class ofBuffer
{
  ...
  public:
    ...
    long size () const;
      ...
      OF_DEPRECATED_MSG ("use a lines iterator instead",
                         string getNextLine ());
      OF_DEPRECATED_MSG ("use a lines iterator instead",
                         string getFirstLine ());
    ...
    {
        Line (vector < char >::iterator _begin,
...
class ofFilePath
{
  public:
    ...
    static string getAbsolutePath (const std::string & path,
                                   bool bRelativeToData = true);

    static bool isAbsolute (const std::string & path);

    static string getFileName (const std::string & filePath,
                               bool bRelativeToData = true);
    static string getBaseName (const std::string & filePath);   // filename without extension

    static string getEnclosingDirectory (const std::string & filePath,
                                         bool bRelativeToData = true);
    static bool createEnclosingDirectory (const std::string & filePath,
                                          bool bRelativeToData =
                                          true, bool bRecursive = true);
    ...
    static string makeRelative (const std::string & from,
                                const std::string & to);
};
...
class ofFile:public fstream
{
  ...
  public:
    ...
    bool copyTo (const std::string & path, bool bRelativeToData =
                 true, bool overwrite = false) const;
    bool moveTo (const std::string & path, bool bRelativeToData =
                 true, bool overwrite = false);
    bool renameTo (const std::string & path, bool bRelativeToData =
                   true, bool overwrite = false);
    ...
    static bool copyFromTo (const std::string & pathSrc,
                            const std::string & pathDst,
                            bool bRelativeToData = true, bool overwrite =
                            false);

    //be careful with slashes here - appending a slash when moving a folder will causes mad headaches in osx
    static bool moveFromTo (const std::string & pathSrc,
                            const std::string & pathDst,
                            bool bRelativeToData = true, bool overwrite =
                            false);
    static bool doesFileExist (const std::string & fPath,
                               bool bRelativeToData = true);
    static bool removeFile (const std::string & path, bool bRelativeToData =
                            true);
...
class ofDirectory
{
  ...
  public:
    ...
    bool copyTo (const string & path, bool bRelativeToData =
                 true, bool overwrite = false);
    bool moveTo (const string & path, bool bRelativeToData =
                 true, bool overwrite = false);
    bool renameTo (const string & path, bool bRelativeToData =
                   true, bool overwrite = false);
    ...
    ofFile getFile (std::size_t position, ofFile::Mode mode =
                    ofFile::Reference, bool binary = false) const;
    ...
    static bool createDirectory (const std::string & dirPath,
                                 bool bRelativeToData = true, bool recursive =
                                 false);
    static bool isDirectoryEmpty (const std::string & dirPath,
                                  bool bRelativeToData = true);
    static bool doesDirectoryExist (const std::string & dirPath,
                                    bool bRelativeToData = true);
    static bool removeDirectory (const std::string & path,
                                 bool deleteIfNotEmpty, bool bRelativeToData =
                                 true);
