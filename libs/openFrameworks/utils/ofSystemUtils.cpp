...
std::string convertWideToNarrow( const std::wstring& wstr, int encoding = CP_UTF8  )
{
    ...
    WideCharToMultiByte                  (encoding, 0, &wstr[0], (int)wstr.size(), &strTo[0], size_needed, NULL, NULL);
    return strTo;
}
...
ofFileDialogResult ofSystemLoadDialog(string windowTitle, bool bFolderSelection, string defaultPath,bool multipleItems){
	...
	@autoreleasepool {
		...
		if(buttonClicked == NSFileHandlingPanelOKButton) {
			...
			for( NSURL* path_files in [loadDialog URLs] )
            {
                results.results.push_back(string([[path_files path]UTF8String]));
            }
	...
	if (bFolderSelection == false){
		...
		if(pidl = SHBrowseForFolderW(&bi)){
			...
			if(SHGetPathFromIDListW(pidl,wideCharacterBuffer)){
				results.filePath = convertWideToNarrow(wideCharacterBuffer,CP_ACP);
			}
			lpMalloc->Free(pidl);
		}
		lpMalloc->Release();
...
string ofSystemTextBoxDialog(string question, string text){
   ...
   WNDCLASSEXW wc;
    ...
    HWND dialog = CreateWindowExW(WS_EX_DLGMODALFRAME,
                                  g_szClassName,
                                  convertNarrowToWide(text).c_str(),
...
string ofSystemPassword(string question, string text){
#if defined( TARGET_LINUX ) && defined (OF_USING_GTK)
    /*
     initGTK();
	...
	[alert setMessageText:[NSString stringWithCString:question.c_str()
											 encoding:NSUTF8StringEncoding]];
	// create secure pass field
	NSSecureTextField* label = [[NSSecureTextField alloc] initWithFrame:NSRectFromCGRect(CGRectMake(0,0,100,40))];
	[label setStringValue:[NSString stringWithCString:text.c_str()
											 encoding:NSUTF8StringEncoding]];
	[alert setAccessoryView:label];
	NSInteger returnCode = [alert runModal];
