...
ofHttpResponse ofSaveURLTo(const std::string& url, const fs::path& path);
...
int ofSaveURLAsync(const std::string& url, const fs::path& path);
...
class ofURLFileLoader  {
    public:
		...
		/// \param path file path to save to
		/// \return HTTP response on success or failure
        ofHttpResponse saveTo(const std::string& url, const fs::path& path);
		...
		/// \param path file path to save to
		/// \returns unique id for the active HTTP request
        int saveAsync(const std::string& url, const fs::path& path);
...
public:
	...
	/// \param path file path to save to
	/// \return HTTP response on success or failure
	virtual ofHttpResponse saveTo(const std::string& url, const fs::path& path)=0;
	...
	/// \param path file path to save to
	/// \returns unique id for the active HTTP request
	virtual int saveAsync(const std::string& url, const fs::path& path)=0;
