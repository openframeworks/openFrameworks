...
bool ofXml::load(const of::filesystem::path & file){
	...
	auto res = auxDoc->load_file(ofToDataPath(file));
	if( res ){
		doc = auxDoc;
		xml = doc->root();
		return true;
...
bool ofXml::parse(const std::string & xmlStr){
	auto auxDoc = std::make_shared<pugi::xml_document>();
		...
		if(auxDoc->load_string(xmlStr)){
	#else
		// if(auxDoc->load(xmlStr.c_str())){
		if(auxDoc->load(xmlStr)){
	#endif
		doc = auxDoc;
		xml = doc->root();
		return true;
...
bool ofXml::save(const of::filesystem::path & file) const{
	if(xml == doc->root()){
		...
		auto res = doc->save_file(ofToDataPath(file));
		...
		if(doc.append_copy(xml.root())){
			...
			return doc.save_file(ofToDataPath(file));
		}
	}
	return false;
}
