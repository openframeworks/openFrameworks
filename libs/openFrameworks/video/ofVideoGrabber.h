...
class ofVideoGrabber:public ofBaseVideoGrabber, public ofBaseVideoDraws
{
  ...
  public:ofVideoGrabber ();
    ...
    {
        return setup (w, h, true);
    }
    bool setup (int w, int h, bool bTexture);
    OF_DEPRECATED_MSG ("Use setup instead", bool initGrabber (int w, int h)
                       {
                       return setup (w, h);
                       }
    )OF_DEPRECATED_MSG ("Use setup instead",
    ...
    template < typename GrabberType > shared_ptr < GrabberType > getGrabber ()
    {
