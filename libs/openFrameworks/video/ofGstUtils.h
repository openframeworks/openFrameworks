...
class ofGstUtils
{
  public:
    ...
    bool setPipelineWithSink (string pipeline, string sinkname =
                              "sink", bool isStream = false);
  ...
  private:
    ...
    {
      public:
        ofGstMainLoopThread ():main_loop (NULL)
        {
        }
...
class ofGstVideoUtils:public ofBaseVideo, public ofGstUtils
{
  public:
    ...
    bool setPipeline (string pipeline, ofPixelFormat pixelFormat =
                      OF_PIXELS_RGB, bool isStream = false, int w =
                      -1, int h = -1);
  ...
  private:
    static gboolean sync_bus_call (GstBus * bus, GstMessage * msg,
                                   gpointer data);
