...
void ofMaterial::setup(const ofMaterialSettings& settings) {
	if (settings.customUniforms != data.customUniforms || settings.postFragment != data.postFragment || settings.mainVertexKey != data.mainVertexKey || settings.mainFragmentKey != data.mainFragmentKey || settings.isPbr != data.isPbr) {
		shaders.clear();
		uniforms1f.clear();
		uniforms2f.clear();
		uniforms3f.clear();
...
void ofMaterial::setTexture(const ofMaterialTextureType& aMaterialTextureType, const ofTexture& aTex) {
	...
	if (aMaterialTextureType == OF_MATERIAL_TEXTURE_CLEARCOAT || aMaterialTextureType == OF_MATERIAL_TEXTURE_CLEARCOAT_ROUGHNESS || aMaterialTextureType == OF_MATERIAL_TEXTURE_CLEARCOAT_INTENSITY_ROUGHNESS) {
		if (aTex.isAllocated()) {
...
void ofMaterial::mergeCustomUniformTextures() {
	mergeCustomUniformTextures(OF_MATERIAL_TEXTURE_ROUGHNESS_METALLIC, { OF_MATERIAL_TEXTURE_ROUGHNESS, OF_MATERIAL_TEXTURE_METALLIC });

	mergeCustomUniformTextures(OF_MATERIAL_TEXTURE_ROUGHNESS_METALLIC, { OF_MATERIAL_TEXTURE_OCCLUSION, OF_MATERIAL_TEXTURE_ROUGHNESS_METALLIC });

	mergeCustomUniformTextures(OF_MATERIAL_TEXTURE_AO_ROUGHNESS_METALLIC, { OF_MATERIAL_TEXTURE_OCCLUSION, OF_MATERIAL_TEXTURE_ROUGHNESS, OF_MATERIAL_TEXTURE_METALLIC });
}
...
void ofMaterial::initShaders(ofGLProgrammableRenderer& renderer) const {
	...
	if (rendererShaders == shaders.end() || rendererShaders->second->numLights != numLights || rendererShaders->second->numCubeMaps != numCubeMaps || rendererShaders->second->shaderId != shaderId) {
