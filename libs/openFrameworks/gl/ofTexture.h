...
OF_DEPRECATED_MSG ("Use member method ofTexture::setTextureWrap() instead.",
                   void ofSetTextureWrap (GLfloat wrapS =
                                          GL_CLAMP_TO_EDGE, GLfloat wrapT =
                                          GL_CLAMP_TO_EDGE));
...
OF_DEPRECATED_MSG
    ("Use member method ofTexture::setTextureMinMagFilter() instead.",
     void ofSetMinMagFilters (GLfloat minFilter =
                              GL_LINEAR, GLfloat magFilter = GL_LINEAR));
...
class ofTexture:public ofBaseDraws
{
  public:
    ...
    virtual void allocate (const ofTextureData & textureData, int glFormat,
                           int pixelType);
    ...
    /// \param pixelType GL pixel type: GL_UNSIGNED_BYTE, GL_FLOAT, etc.
    virtual void allocate (int w, int h, int glInternalFormat, int glFormat,
                           int pixelType);
    ...
    virtual void allocate (int w, int h, int glInternalFormat,
                           bool bUseARBExtension, int glFormat,
                           int pixelType);
    ...
    void allocateAsBufferTexture (const ofBufferObject & buffer,
    ...
    void loadData (const unsigned char *const data, int w, int h,
                   int glFormat);
    ...
    void draw (const ofPoint & p1, const ofPoint & p2, const ofPoint & p3,
               const ofPoint & p4) const;
    ...
    ofMesh getQuad (const ofPoint & p1, const ofPoint & p2,
                    const ofPoint & p3, const ofPoint & p4) const;
    ...
    ofMesh getMeshForSubsection (float x, float y, float z, float w, float h,
    ...
    void bindAsImage (GLuint unit, GLenum access, GLint level =
                      0, GLboolean layered = 0, GLint layer = 0);
#endif
