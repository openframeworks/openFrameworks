...
ofColor_<PixelType>& ofColor_<PixelType>::normalize() {
	float scaleFactor = getBrightness() / flimit();
	*this /= scaleFactor;
	return *this;
}
...
void ofColor_<PixelType>::getHsb(float& hue,
	...
	hue = flimit() * hueSixth / 6.f;
	saturation = flimit() * (max - min) / max;
	brightness = max;
}
...
void ofColor_<PixelType>::setHsb(float hue, float saturation, float brightness, PixelType alpha) {
	saturation = ofClamp(saturation, 0.f, flimit());
	brightness = ofClamp(brightness, 0.f, flimit());
	if(brightness == 0) { // black
		set(0);
	} else if(saturation == 0) { // grays
		set(brightness);
	} else {
		float hueSix = hue * 6.f / flimit();
		float saturationNorm = saturation / flimit();
...
template<typename PixelType>
ofColor_<PixelType> & ofColor_<PixelType>::operator *= (const ofColor_<PixelType>& color){
	r = clampedMultiply(r, (float) color.r / flimit());
	g = clampedMultiply(g, (float) color.g / flimit());
	b = clampedMultiply(b, (float) color.b / flimit());
	return *this;
}
...
template<typename PixelType>
ofColor_<PixelType> & ofColor_<PixelType>::operator /= (const ofColor_<PixelType>& color){
	r = clampedDivide(r, (float) color.r / flimit());
	g = clampedDivide(g, (float) color.g / flimit());
	b = clampedDivide(b, (float) color.b / flimit());
	return *this;
}
