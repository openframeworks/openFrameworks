...
/// \tparam T The pixel data type.
template < typename T > class ofBaseImage_:public ofAbstractImage,
    virtual public ofBaseHasPixels_ <
    T >
{
  public:
...
class ofBaseSoundInput
{
  public:
    ...
    virtual void audioIn (float *input, int bufferSize, int nChannels,
                          int deviceID, long unsigned long tickCount);
...
class ofBaseVideoDraws:virtual public ofBaseVideo,
    public ofBaseDraws,
    public ofBaseHasTexturePlanes, virtual public ofBaseHasPixels
{
  public:
...
class ofBaseRenderer
{
  public:
    ...
    {
        draw (mesh, renderType, mesh.usingColors (), mesh.usingTextures (),
              mesh.usingNormals ());
    }
    virtual void draw (const ofMesh & vertexData, ofPolyRenderMode renderType,
                       bool useColors, bool useTextures,
                       bool useNormals) const = 0;
    virtual void draw (const of3dPrimitive & model,
                       ofPolyRenderMode renderType) const = 0;
    virtual void draw (const ofNode & model) const = 0;
    virtual void draw (const ofImage & image, float x, float y, float z,
                       float w, float h, float sx, float sy, float sw,
                       float sh) const = 0;
    virtual void draw (const ofFloatImage & image, float x, float y, float z,
                       float w, float h, float sx, float sy, float sw,
                       float sh) const = 0;
    virtual void draw (const ofShortImage & image, float x, float y, float z,
                       float w, float h, float sx, float sy, float sw,
                       float sh) const = 0;
    virtual void draw (const ofBaseVideoDraws & video, float x, float y,
    ...
    virtual void viewport (float x = 0, float y = 0, float width =
                           -1, float height = -1, bool vflip = true) = 0;
    virtual void setupScreenPerspective (float width = -1, float height =
                                         -1, float fov = 60, float nearDist =
                                         0, float farDist = 0) = 0;
    virtual void setupScreenOrtho (float width = -1, float height =
                                   -1, float nearDist = -1, float farDist =
                                   1) = 0;
    ...
    virtual void bind (const ofCamera & camera,
                       const ofRectangle & viewport) = 0;
    ...
    // drawing
    virtual void drawLine (float x1, float y1, float z1, float x2, float y2,
                           float z2) const = 0;
    virtual void drawRectangle (float x, float y, float z, float w,
                                float h) const = 0;
    virtual void drawTriangle (float x1, float y1, float z1, float x2,
                               float y2, float z2, float x3, float y3,
                               float z3) const = 0;
    virtual void drawCircle (float x, float y, float z, float radius) const =
        0;
    virtual void drawEllipse (float x, float y, float z, float width,
                              float height) const = 0;
    virtual void drawString (string text, float x, float y, float z) const =
        0;
    virtual void drawString (const ofTrueTypeFont & font, string text,
    ...
    virtual void drawPlane (float x, float y, float width,
                            float height) const;
    virtual void drawPlane (float x, float y, float z, float width,
                            float height) const;
    virtual void drawPlane (ofPoint & position, float width,
                            float height) const;
    ...
    virtual void drawIcoSphere (float x, float y, float z,
                                float radius) const;
    ...
    // Cylinder //
    virtual void setCylinderResolution (int radiusSegments,
                                        int heightSegments, int capSegments =
                                        2);
    virtual ofVec3f getCylinderResolution () const;
    virtual void drawCylinder (float x, float y, float radius,
                               float height) const;
    virtual void drawCylinder (float x, float y, float z, float radius,
                               float height) const;
    virtual void drawCylinder (const ofPoint & position, float radius,
                               float height) const;
    ...
    virtual void setConeResolution (int radiusSegments, int heightSegments,
                                    int capSegments = 2);
    ...
    virtual void drawCone (float x, float y, float z, float radius,
                           float height) const;
    virtual void drawCone (float x, float y, float radius,
                           float height) const;
    virtual void drawCone (const ofPoint & position, float radius,
                           float height) const;
    ...
    virtual void drawBox (float x, float y, float z, float width,
                          float height, float depth) const;
    ...
    /// \param height The height of the box.
    /// \param depth The depth of the box.
    virtual void drawBox (const ofPoint & position, float width, float height,
                          float depth) const;
    ...
    virtual void drawGrid (float stepSize, size_t numberOfSteps, bool labels,
                           bool x, bool y, bool z) const;
    virtual void drawGridPlane (float stepSize, size_t numberOfSteps,
                                bool labels) const;
    virtual void drawArrow (const ofVec3f & start, const ofVec3f & end,
                            float headSize) const;
    virtual void drawRotationAxes (float radius, float stripWidth,
                                   int circleRes) const;
};
...
class ofBaseGLRenderer:public ofBaseRenderer
{
  public:
    ...
    virtual void draw (const ofTexture & image, float x, float y, float z,
                       float w, float h, float sx, float sy, float sw,
                       float sh) const = 0;
    virtual void draw (const ofVbo & vbo, GLuint drawMode, int first,
                       int total) const = 0;
    virtual void drawElements (const ofVbo & vbo, GLuint drawMode, int amt,
                               int offsetelements) const = 0;
    virtual void drawInstanced (const ofVbo & vbo, GLuint drawMode, int first,
                                int total, int primCount) const = 0;
    virtual void drawElementsInstanced (const ofVbo & vbo, GLuint drawMode,
                                        int amt, int primCount) const = 0;
    virtual void draw (const ofVboMesh & mesh,
                       ofPolyRenderMode renderType) const = 0;
    virtual void drawInstanced (const ofVboMesh & mesh,
                                ofPolyRenderMode renderType,
                                int primCount) const = 0;


    virtual void enableTextureTarget (const ofTexture & tex,
                                      int textureLocation) = 0;
    virtual void disableTextureTarget (int textureTarget,
                                       int textureLocation) = 0;
    ...
    virtual void setLightSpotConcentration (int lightIndex, float exponent) =
        0;
    virtual void setLightAttenuation (int lightIndex, float constant,
                                      float linear, float quadratic) = 0;
    virtual void setLightAmbientColor (int lightIndex,
                                       const ofFloatColor & c) = 0;
    virtual void setLightDiffuseColor (int lightIndex,
                                       const ofFloatColor & c) = 0;
    virtual void setLightSpecularColor (int lightIndex,
                                        const ofFloatColor & c) = 0;
    virtual void setLightPosition (int lightIndex, const ofVec4f & position) =
        0;
    virtual void setLightSpotDirection (int lightIndex,
                                        const ofVec4f & direction) = 0;

    virtual int getGLVersionMajor () = 0;
    virtual int getGLVersionMinor () = 0;

    virtual void saveScreen (int x, int y, int w, int h, ofPixels & pixels) =
        0;
    ...
    virtual void bindForBlitting (const ofFbo & fboSrc, ofFbo & fboDst,
                                  int attachmentPoint = 0) = 0;
#endif
...
class ofBaseURLFileLoader
{
  public:
    ...
    virtual ofHttpResponse saveTo (const string & url, const string & path) =
        0;
...
class ofBaseMaterial
{
  public:
    ...
    virtual const ofShader & getShader (int textureTarget,
                                        ofGLProgrammableRenderer & renderer)
        const = 0;
    virtual void uploadMatrices (const ofShader & shader,
                                 ofGLProgrammableRenderer & renderer) const;
    virtual void updateMaterial (const ofShader & shader,
                                 ofGLProgrammableRenderer & renderer) const =
        0;
    virtual void updateLights (const ofShader & shader,
