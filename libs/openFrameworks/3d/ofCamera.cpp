...
glm::mat4 ofCamera::getProjectionMatrix(const ofRectangle & viewport) const {
	...
	if (isOrtho) {
		return glm::translate(glm::mat4(1.0), { -lensOffset.x, -lensOffset.y, 0.f }) * glm::ortho(-viewport.width / 2, +viewport.width / 2, -viewport.height / 2, +viewport.height / 2, nearClip, farClip);
	} else {
