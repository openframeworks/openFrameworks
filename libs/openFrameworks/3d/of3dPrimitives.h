...
class ofConePrimitive:public of3dPrimitive
{
  public:
    ...
    void set (float radius, float height, int radiusSegments,
              int heightSegments, int capSegments = 2, ofPrimitiveMode mode =
              OF_PRIMITIVE_TRIANGLE_STRIP);
...
class ofBoxPrimitive:public of3dPrimitive
{
  public:
    ...
    enum BoxSides
    {
      ...
      ofBoxPrimitive ();
      ofBoxPrimitive (float width, float height, float depth, int resWidth =
                      2, int resHeight = 2, int resDepth = 2);
     ~ofBoxPrimitive ();
