...
private:
	...
	bool reportAppVeyor(bool passed, uint64_t durationMs) {
		...
		if (ofGetEnv(APPVEYOR_API_URL) != "") {
			...
			req.body = "{ " + json_var_value("testName", projectName.string()) + ", " + json_var_value("testFramework", "ofxUnitTests") + ", " + json_var_value("fileName", exeName.string()) + ", " + json_var_value("outcome", passed ? "Passed" : "Failed") + ", " + json_var_value("durationMilliseconds", ofToString(durationMs)) + ", " + json_var_value("StdOut", stdOut) + ", " + json_var_value("StdErr", stdErr) + "}";
