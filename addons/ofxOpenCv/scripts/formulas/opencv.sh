...
function download() {
  if [ $TYPE = android ]; then
      curl -Lk "http://downloads.sourceforge.net/project/opencvlibrary/opencv-android/$VER/OpenCV-$VER-android-sdk.zip?ts=1413205907&use_mirror=optimate" -o opencv-$VER.zip
      unzip opencv-$VER.zip
      mv OpenCV-$VER-android-sdk opencv
      rm opencv-$VER.zip
  else
...
function build_android() {
  ...
  ABI=$1
  targetFolder="$LIB_FOLDER/$TYPE/$ABI"
  local targetFolder=$BUILD_DIR/opencv/build/$TYPE/$ABI
  source ../../formulas/android_configure.sh $ABI
  mkdir -p build
  cd build
    cmake .. -DCMAKE_INSTALL_PREFIX="$targetFolder" \
      -DGLFW_BUILD_UNIVERSAL=ON \
      -DCMAKE_CXX_FLAGS="-fvisibility-inlines-hidden -O3" \
      -DCMAKE_C_FLAGS="-fvisibility-inlines-hidden -O3" \
      -DCMAKE_BUILD_TYPE="Release" \
      -DBUILD_SHARED_LIBS=OFF \
      -DBUILD_DOCS=OFF \
      -DBUILD_EXAMPLES=OFF \
...
function copy() {
  ...
  if [ -f $1/lib/$TYPE/libopencv_core.a ]; then
      rm $1/lib/$TYPE/*.a
  fi
  mkdir -p $1/lib/$TYPE
  ...
  if [ "$TYPE" == "android" ]; then
    # Standard *nix style copy.
    # copy headers
    cp -R sdk/native/jni/include/* $1/include/
 
    # copy lib
    if [ -f $1/lib/$TYPE/armeabi-v7a/libopencv_core.a ]; then
      rm $1/lib/$TYPE/*/*.a
    fi
