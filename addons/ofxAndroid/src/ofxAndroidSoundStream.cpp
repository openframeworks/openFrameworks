...
bool ofxAndroidSoundStream::setup(const ofSoundStreamSettings & settings);
	...
	int input_buffer_size = inChannels*getMinInBufferSize(settings.sampleRate,settings.numInputChannels) * 2;
	...
	totalOutRequestedBufferSize = settings.bufferSize*settings.outChannels;
	totalInRequestedBufferSize = settings.bufferSize*numInputChannels;
