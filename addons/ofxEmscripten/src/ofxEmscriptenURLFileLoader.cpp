...
ofHttpResponse ofxEmscriptenURLFileLoader::saveTo(const string &  url, const filesystem::path &  path){
	saveAsync(url,path);
	return ofHttpResponse();
}

int ofxEmscriptenURLFileLoader::saveAsync(const string &  url, const filesystem::path &  path){
