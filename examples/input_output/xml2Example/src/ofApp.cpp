...
void ofApp::setup(){
	...
	auto drawing = xml.child("drawing");
	if(!drawing){
	...
	if(!bg){
		...
		// create the correct background tag and add colors
	...
	if(background){ // this will always be true, only to document how to search
		bgColor.r = background.child("red").intValue();
		bgColor.g = background.child("green").intValue();
		bgColor.b = background.child("blue").intValue();
	}
	...
	for(auto & stroke: strokesXml){
		...
		// vertex in the mesh
		auto pts = stroke.children("pt");
		for(auto & pt: pts){
			auto x = pt.attribute("x").intValue();
			auto y = pt.attribute("y").intValue();
...
void ofApp::mouseDragged(int x, int y, int button){
	...
	bg.child("red").set(bgColor.r);
	bg.child("green").set(bgColor.g);
	bg.child("blue").set(bgColor.b);
...
void ofApp::mousePressed(int x, int y, int button){
	...
	currentStroke = xml.child("drawing").insertChildBefore("stroke", bg);
	strokes.emplace_back();
	strokes.back().setMode(OF_PRIMITIVE_LINE_STRIP);
	xmlChanged = true;
}
