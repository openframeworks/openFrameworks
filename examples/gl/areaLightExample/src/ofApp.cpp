...
void ofApp::update() {
    ...
    if( mode == 0 ) {
        ...
        areaLight.setPosition(cos(orbitSpeed)*-600,(sin(etimef*1.1)*200), sin(orbitSpeed)*-600);
        areaLight.lookAt(glm::vec3(0, -150, 0));
        areaLight.setAmbientColor(ofFloatColor(0.3,0.3,0.3));
    } else if( mode == 1 ) {
        float hpct = ofMap( ofGetMouseX(), 100, ofGetWidth()-100, 0, 1, true );
        highwaySpeed = ofMap( hpct, 0, 1, 500, 5000, true );
        areaLight.setAmbientColor(ofFloatColor(0.0,0.0,0.0));
        float tz = ofClamp(sin(etimef), 0, 1) * 500.0+200;
        areaLight.setPosition( areaLight.getPosition().x + deltaTime * highwaySpeed, 350, tz );
        if( areaLight.getPosition().x > 1500 ) {
            areaLight.setPosition(-1500, areaLight.getPosition().y, tz );
        }
        ...
        float tnoise = ofSignedNoise(areaLight.getPosition().y * hpct * 0.001, areaLight.getPosition().x * hpct * 0.001, hpct * etimef );
        if( tnoise > 1.0 - hpct * 0.5 && bump > 1.0 ) {
            bump = 0.0;
            bumpHeight = ofRandom(10, 80);
        }
        bump += deltaTime * (3.0 + (hpct+0.2));
    } else if( mode == 2 ) {
        float hpct = ofMap( ofGetMouseX(), 100, ofGetWidth()-100, 0, 1, true );
        areaLight.setPosition(cos(etimef*2.0)*-600,fabs(sin(etimef*2.0)*600)-200,sin(etimef*3.4f)*200+400);
//        areaLight.rotateDeg(-90,glm::vec3(1,0,0));
        areaLight.tiltDeg(deltaTime*(hpct*360.0*3.0+5.0f));
    }
}
...
void ofApp::draw(){
    ...
    materialMesh.begin(); {
        ofPushMatrix(); {
            ...
            if( mode == 1 ) {
                if( bump < 1.0 ) {
                    ofTranslate(0.0, sin(bump * PI) * bumpHeight, 0.0 );
                }
            }
            ofRotateXDeg(-90);
            logoMesh.draw();
