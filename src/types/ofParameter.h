...
private:
	...
	friend class ofDerivedParameter;
...
template<typename T, typename U>
class ofDerivedParameter: public ofAbstractParameter{
public:
	ofDerivedParameter(const std::string & name, const ofParameter<T> & v, std::function<U(const T &)>);
	...
	template<class ListenerClass, typename ListenerMethod>
	void addListener(ListenerClass * listener, ListenerMethod method, int prio=OF_EVENT_ORDER_AFTER_APP){
		ofAddListener(changedE,listener,method,prio);
	}

	template<class ListenerClass, typename ListenerMethod>
	void removeListener(ListenerClass * listener, ListenerMethod method, int prio=OF_EVENT_ORDER_AFTER_APP){
		ofRemoveListener(changedE,listener,method,prio);
	}

	template<typename... Args>
	ofEventListener newListener(Args...args) {
		return changedE.newListener(args...);
	}
	...
	void setMin(const U & min);
	void setMax(const U & max);
	...
	const ofParameterGroup getFirstParent() const{
		auto first = std::find_if(parents.begin(),parents.end(),[](weak_ptr<ofParameterGroup::Value> p){return p.lock()!=nullptr;});
		if(first!=parents.end()){
			return first->lock();
		}else{
			return shared_ptr<ofParameterGroup::Value>(nullptr);
		}
...
private:
	...
	ofParameter<T> mom;
	vector<weak_ptr<ofParameterGroup::Value>> parents;
	std::function<U(const T & v)> mapping;
};

template<typename T, typename U>
ofDerivedParameter<T,U>::ofDerivedParameter(const std::string & name, const ofParameter<T> & v, std::function<U(const T &)> mapping)
:name(name)
,mom(v)
,mapping(mapping){
...
template<typename T, typename U>
const U ofDerivedParameter<T,U>::get() const{
	return mapping(mom);
}

template<typename T, typename U>
ofDerivedParameter<T,U>::operator const U() const{
	return mapping(mom);
}

template<typename T, typename U>
void ofDerivedParameter<T,U>::setName(const std::string & name){
	this->name = name;
}

template<typename T, typename U>
std::string ofDerivedParameter<T,U>::getName() const{
	return name;
}

template<typename T, typename U>
U ofDerivedParameter<T,U>::getMin() const{
	return min;
}

template<typename T, typename U>
U ofDerivedParameter<T,U>::getMax() const{
	return max;
}

template<typename T, typename U>
std::string ofDerivedParameter<T,U>::toString() const{
	return ofToString(get());
}

template<typename T, typename U>
void ofDerivedParameter<T,U>::enableEvents(){
	changedE.enable();
}

template<typename T, typename U>
void ofDerivedParameter<T,U>::disableEvents(){
	changedE.disable();
}

template<typename T, typename U>
bool ofDerivedParameter<T,U>::isSerializable() const{
	return false;
}

template<typename T, typename U>
bool ofDerivedParameter<T,U>::isReadOnly() const{
	return true;
}

template<typename T, typename U>
void ofDerivedParameter<T,U>::setMin(const U & min){
	this->min = min;
}

template<typename T, typename U>
void ofDerivedParameter<T,U>::setMax(const U & max){
	this->max = max;
}

template<typename T, typename U>
std::shared_ptr<ofAbstractParameter> ofDerivedParameter<T,U>::newReference() const{
	return std::make_shared<ofDerivedParameter<T,U>>(*this);
}

template<typename T, typename U>
void ofDerivedParameter<T,U>::setParent(ofParameterGroup & _parent){
	parents.emplace_back(_parent.obj);
}

template<typename T, typename U>
std::size_t ofDerivedParameter<T,U>::getNumListeners() const{
	return changedE.size();
}
