...
texture3DS::texture3DS(string filename, const int textureId){
    ...
    if( !img.loadImage(filename) ){
		ofLog(OF_ERROR, "texture3DS ERROR:  Could not open %s", filename.c_str());
    }
    ...
    if( m_width <= 0 || m_height <= 0){
		ofLog(OF_ERROR, "texture3DS ERROR: Something wrong with %s - dimensions less than 0", filename.c_str());

    }

    if(m_bpp != 32 && m_bpp != 24){		
		ofLog(OF_ERROR, "texture3DS ERROR: Invalid texture color depth %s  must be uncompressed 24/32bpp png, jpg, bmp, tga", filename.c_str());
        return;
    }
    ...
    switch(m_bpp){
        ...
        default:
			ofLog(OF_ERROR, "texture3DS ERROR: Invalid texture color depth %s  must be uncompressed 24/32bpp", filename.c_str());
    ...
    ofLog(OF_NOTICE, "texture3DS Texture %s loaded", filename.c_str());
}
