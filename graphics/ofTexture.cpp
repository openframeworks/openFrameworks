...
void ofTexture::allocate(int w, int h, int internalGlDataType,
	...
	glTexImage2D(texData.textureTarget, 0, texData.glTypeInternal,
			texData.tex_w, texData.tex_h, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0); // init to black...
#else
...
void ofTexture::loadData(unsigned char * data, int w, int h, int glDataType) {
	...
	if (w > texData.tex_w || h > texData.tex_h) {
		ofLog(OF_ERROR,
				"image data too big for allocated texture. not uploading...");
		return;
	}
	...
	glEnable(texData.textureTarget);
	glBindTexture(texData.textureTarget, (GLuint) texData.textureName[0]);
	glTexSubImage2D(texData.textureTarget, 0, 0, 0, w, h, texData.glType,
...
void ofTexture::draw(float x, float y, float w, float h) {
	...
	GLfloat tex_coords[] = { tx0, ty0, tx1, ty0, tx1, ty1, tx0, ty1 };
	GLfloat verts[] = { px0, py0, px1, py0, px1, py1, px0, py1 };
