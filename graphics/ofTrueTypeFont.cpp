...
static vector<ofPoint> ofSimplifyContour(vector<ofPoint> &V, float tol) {
	...
	if (m < sV.size())
		sV.erase(sV.begin() + m, sV.end());
	return sV;
}

//------------------------------------------------------------
static void quad_bezier(vector<ofPoint> &ptsList, float x1, float y1, float x2,
		float y2, float x3, float y3, int res) {
	for (int i = 0; i <= res; i++) {
...
//-----------------------------------------------------------
static void cubic_bezier(vector<ofPoint> &ptsList, float x0, float y0,
		float x1, float y1, float x2, float y2, float x3, float y3, int res) {
	float ax, bx, cx;
	float ay, by, cy;
	float t, t2, t3;
	float x, y;
...
static ofTTFCharacter makeContoursForCharacter(FT_Face &face) {
	...
	for (int k = 0; k < nContours; k++) {
		...
		for (int j = startPos; j < endPos; j++) {
			...
			if (FT_CURVE_TAG(tags[j]) == FT_CURVE_TAG_ON) {
				...
				if (FT_CURVE_TAG(tags[j]) == FT_CURVE_TAG_CUBIC) {
					...
					ofPoint nextPoint((float) vec[nextIndex].x,
							-(float) vec[nextIndex].y);

					//we need two control points to draw a cubic bezier
					bool lastPointCubic = (FT_CURVE_TAG(tags[prevPoint])
							!= FT_CURVE_TAG_ON)
							&& (FT_CURVE_TAG(tags[prevPoint])
									== FT_CURVE_TAG_CUBIC);

					if (lastPointCubic) {
						ofPoint controlPoint1(vec[prevPoint].x,
								-vec[prevPoint].y);
						ofPoint controlPoint2(vec[j].x, -vec[j].y);
						ofPoint nextPoint((float) vec[nextIndex].x,
								-(float) vec[nextIndex].y);

						cubic_bezier(testOutline, lastPoint.x, lastPoint.y,
								controlPoint1.x, controlPoint1.y,
								controlPoint2.x, controlPoint2.y, nextPoint.x,
								nextPoint.y, 8);
					}
					...
					if (j == startPos) {
						bool prevIsConnic = (FT_CURVE_TAG( tags[endPos-1] )
								!= FT_CURVE_TAG_ON)
								&& (FT_CURVE_TAG( tags[endPos-1])
										!= FT_CURVE_TAG_CUBIC);

						if (prevIsConnic) {
							ofPoint lastConnic(vec[endPos - 1].x, -vec[endPos
									- 1].y);
					...
					bool nextIsConnic = (FT_CURVE_TAG( tags[nextIndex] )
							!= FT_CURVE_TAG_ON)
							&& (FT_CURVE_TAG( tags[nextIndex])
									!= FT_CURVE_TAG_CUBIC);
					...
					quad_bezier(testOutline, lastPoint.x, lastPoint.y,
							conicPoint.x, conicPoint.y, nextPoint.x,
							nextPoint.y, 8);
		...
		if (testOutline.size()) {
			charOutlines.contours.back().pts = ofSimplifyContour(testOutline,
...
//------------------------------------------------------------------
void ofTrueTypeFont::loadFont(string filename, int fontsize,
		bool _bAntiAliased, bool _bFullCharacterSet, bool makeContours) {
	...
	for (int i = 0; i < nCharacters; i++) {
		...
		//------------------------------------------ anti aliased or not:
		if (FT_Load_Glyph(face, FT_Get_Char_Index(face, (unsigned char) (i
				+ NUM_CHARACTER_TO_START)), FT_LOAD_DEFAULT)) {
		...
		// texture internals
		cps[i].tTex = (float) (bitmap.width + visibleBorder * 2)
				/ (float) width;
		cps[i].vTex = (float) (bitmap.rows + visibleBorder * 2)
				/ (float) height;
...
ofTTFCharacter ofTrueTypeFont::getCharacterAsPoints(int character) {
	...
	if (bMakeContours && charOutlines.size() > 0 && character
			>= NUM_CHARACTER_TO_START && character - NUM_CHARACTER_TO_START
			< charOutlines.size()) {
...
void ofTrueTypeFont::drawChar(int c, float x, float y) {
	...
	if (glIsTexture(texNames[cu])) {
		...
		GLfloat verts[] = { x2, y2, x2, y1, x1, y1, x1, y2 };
		GLfloat tex_coords[] = { t2, v2, t2, v1, t1, v1, t1, v2 };
...
void ofTrueTypeFont::drawCharAsShape(int c, float x, float y) {
	...
	if (!bMakeContours) {
		ofLog(
				OF_ERROR,
				"Error : contours not created for this font - call loadFont with makeContours set to true");
		return;
	}
	...
	for (int k = 0; k < charRef.contours.size(); k++) {
		...
		for (int i = 0; i < charRef.contours[k].pts.size(); i++) {
			ofVertex(charRef.contours[k].pts[i].x + x,
					charRef.contours[k].pts[i].y + y);
		}
	}
	ofEndShape(true);
