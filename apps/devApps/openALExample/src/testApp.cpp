...
void testApp::draw(){
	...
	float * spectrum = ofOpenALSoundPlayer::getSystemSpectrum(512);
	//float * spectrum = ofFmodSoundGetSpectrum(512);
	for(int i=0;i<512;i++){
		ofRect(i*2,0,2,spectrum[i]*1000);
	}
...
void testApp::mousePressed(int x, int y, int button){
	...
	if (x < widthStep){
		...
		vocals.setPan(((float)x - widthStep*2.f)/ widthStep);
	}
}
